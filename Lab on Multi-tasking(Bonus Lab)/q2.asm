...
pcb:		
		dw 0, 0, 0, 0, 0 ; task0 regs[cs:pcb + 0]
		dw 0, 0, 0, 0, 0 ; task1 regs start at [cs:pcb + 10]
		dw 0, 0, 0, 0, 0 ; task2 regs start at [cs:pcb + 20]
		dw 0, 0, 0, 0, 0 ; task1 regs start at [cs:pcb + 30]
		dw 0, 0, 0, 0, 0 ; task2 regs start at [cs:pcb + 40]
...
taskthree:	mov al, [chars+bx]			; read the next shape
			mov [es:3200], al				; write at top left of screen 
			inc bx						; increment to next shape
			and bx, 3					; taking modulus by 4
...
;---------------------------------------------------------------------------
taskfour:	mov al, [chars+bx]			; read the next shape...0
			mov [es:3318], al			; write at top right of screen
			inc bx						; increment to next shape
			and bx, 3					; taking modulus by 4
			jmp taskfour					; infinite task
